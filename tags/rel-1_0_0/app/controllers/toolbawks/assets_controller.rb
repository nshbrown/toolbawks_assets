class Toolbawks::AssetsController < Toolbawks::BaseController
  ...
  def show
    filter = (params[:filter] == "true") ? filter_settings(params[:association][:klass], params[:association][:id]) : nil
    ...
    def filter_settings(klass, id)
      ...
      case config[:association].macro
        when :has_and_belongs_to_many
          ...
          filter[:conditions] = "#{config[:column]} = #{id}"
        when :belongs_to
        when :has_many
      end
